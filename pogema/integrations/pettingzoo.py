...
class PogemaParallel:
    ...
    def __init__(self, grid_config: GridConfig):
        self.metadata = {'render_modes': ['human'], "name": "pogema"}
    ...
    def step(self, actions):
        ...
        observations, rewards, dones, infos = self.pogema.step(actions)
        ...
        d_dones = {agent: dones[anm[agent]] for agent in self.agents}
        d_infos = {agent: infos[anm[agent]] for agent in self.agents}

        for agent, idx in anm.items():
            if dones[idx] and agent in self.agents:
                self.agents.remove(agent)

        return d_observations, d_rewards, d_dones, d_infos
